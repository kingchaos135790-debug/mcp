...
- `workspace_patch_summary` for read-only Git patch summaries.
- `session_edit_context` for changed files and suggested fresh read ranges during multi-step edit sessions.

Still open / lower priority:

- PowerShell stdout/stderr capture improvements for nested inline scripts. This appears to belong to the outer Windows MCP tool transport layer, not `windows-code-search-mcp`.
- Full persistent edit-context tracking across process restarts.
